...
# -*- coding: utf-8 -*-
"""
Flask Web 猜数字 · 强化版（已修复：TemplateAssertionError block 'content' defined twice）
- 表单校验（白名单/长度/数值范围）
- CSRF 保护（Flask-WTF）
- 排行榜：SQLite（默认）或 JSON（文件锁）
- Bootstrap 5 UI + 进度条
...
def load_board_sqlite():
    """返回 {label: [ {name, attempts, range, date}, ... ]}，每组 Top 10（按 attempts 升序）"""
    conn = sqlite3.connect(DB_PATH)
    ...
    try:
        ...
        for lbl in labels:
            rows = conn.execute(
                "SELECT name, attempts, range_low, range_high, created_at FROM scores "
                "WHERE label=? ORDER BY attempts ASC, id ASC LIMIT 10",
                (lbl,),
            ).fetchall()
            board[lbl] = [
                {
...
# 注意：这里使用 {{ content|safe }} 占位，避免重复定义 block
BASE_HTML = """
<!doctype html>
<html lang="zh">
<head>
...
def play():
    ...
    if request.method == "POST":
        # 手动处理（也可用 form.validate_on_submit，这里保守写法兼容性更好）
    ...
    body = """
      ...
      {% if not done %}
        <form method="post" class="row g-2 align-items-center">
          <div class="col-auto">
            <input class="form-control" type="number" name="guess" placeholder="请输入数字" required>
          </div>
          <div class="col-auto">
            <button class="btn btn-primary" type="submit">提交</button>
          </div>
          <div class="col-auto">
            <button class="btn btn-outline-secondary" name="reset" value="1">重开</button>
          </div>
        </form>
        <p class="mt-2">{{ msg }}</p>
      {% else %}
        ...
        <form method="post" action="{{ url_for('submit_score') }}" class="row g-2 align-items-center">
          <div class="col-auto">
            <input class="form-control" type="text" name="name" placeholder="留名上榜（可空）">
          </div>
          <div class="col-auto">
            <input class="btn btn成功" type="submit" value="提交成绩">
          </div>
        </form>
...
def submit_score():
    ...
    name = (request.form.get("name") or "").strip() or "匿名玩家"
    attempts = session.get("attempts", 0)
    low0, high0, label = session["low0"], session["high0"], session["label"]
